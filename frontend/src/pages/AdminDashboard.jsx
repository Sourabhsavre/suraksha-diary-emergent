...
import { Bell, BellRing, LogOut, Search, Filter, MapPin, FileDown, Shield, AlertTriangle, CheckCircle2, Eye, Clock, RefreshCw, X } from 'lucide-react';
...
const STATUS = {
  new: { label: 'नई', color: 'bg-yellow-100 text-yellow-800 border-yellow-500', icon: Clock },
  in_progress: { label: 'देखी जा रही है', color: 'bg-orange-100 text-orange-800 border-orange-500', icon: Eye },
  resolved: { label: 'सुलझ गई', color: 'bg-emerald-100 text-emerald-800 border-emerald-500', icon: CheckCircle2 },
};
...
export default function AdminDashboard() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (lastIdsRef.current.size > 0 && newOnes.length > 0) {
        ...
        toast[anyUrgent ? 'error' : 'info'](`${newOnes.length} नई रिपोर्ट${anyUrgent ? ' — तुरंत ज़रूरी!' : ''}`);
      }
  ...
  const enablePush = async () => {
    try {
      if (!('serviceWorker' in navigator) || !('PushManager' in window)) { toast.error('Push not supported'); return; }
  ...
  const download = async (kind) => {
    const t = localStorage.getItem(SESSION_STORAGE_KEY);
    const r = await fetch(`${API}/export/${kind}`, { headers: { 'X-Session-Token': t || '' } });
    ...
    <div className="min-h-[100dvh] font-dash bg-[#FDFBF7] relative z-10">
      ...
      <header className="bg-[#0F172A] text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-heading text-xl">सुरक्षा डायरी — कार्यालय</div>
            <div className="text-xs text-slate-300">Suraksha Diary Admin Dashboard</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button data-testid="enable-push-btn" onClick={enablePush} variant="outline" className="border-slate-500 text-white bg-transparent hover:bg-white/10">{pushOn ? <BellRing className="w-4 h-4 mr-1" /> : <Bell className="w-4 h-4 mr-1" />} Notify</Button>
      ...
      <div className="px-6 py-5">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
          <Stat label="कुल" value={stats.total} />
          <Stat label="तुरंत ज़रूरी" value={stats.urgent} accent="text-red-700 border-red-500 bg-red-50" />
          <Stat label="नई" value={stats.new} accent="text-yellow-800 border-yellow-500 bg-yellow-50" />
          <Stat label="देखी जा रही है" value={stats.inProgress} accent="text-orange-800 border-orange-500 bg-orange-50" />
          <Stat label="सुलझ गई" value={stats.resolved} accent="text-emerald-800 border-emerald-500 bg-emerald-50" />
        </div>
        ...
        <div className="bg-white border-2 border-[#0F172A]/20 rounded-2xl p-4 mb-4 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2"><Filter className="w-4 h-4" />
            <Select value={status} onValueChange={setStatus}>
              ...
              <SelectContent>
                <SelectItem value="all">सभी स्थितियाँ</SelectItem>
                <SelectItem value="new">नई</SelectItem>
                <SelectItem value="in_progress">देखी जा रही है</SelectItem>
                <SelectItem value="resolved">सुलझ गई</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <Button data-testid="urgent-only-btn" onClick={() => setUrgentOnly((v) => !v)} className={`h-11 ${urgentOnly ? 'bg-red-600 text-white' : 'bg-white text-[#0F172A] border-2 border-[#0F172A]/40'}`}><AlertTriangle className="w-4 h-4 mr-1" /> तुरंत ज़रूरी</Button>
          <div className="flex-1 min-w-56 relative">
            <Search className="w-4 h-4 absolute left-3 top-3.5 text-slate-500" />
            <Input data-testid="search-input" value={q} onChange={(e) => setQ(e.target.value)} placeholder="खोजें — नाम, विवरण, स्थान…" className="h-11 pl-9 border-2" />
          </div>
          <Button data-testid="refresh-btn" onClick={refresh} variant="outline" className="h-11"><RefreshCw className="w-4 h-4 mr-1" /> Refresh</Button>
        </div>

        <div className="grid gap-3">
          {items.length === 0 && <div className="p-8 text-center text-slate-500 bg-white rounded-2xl border-2 border-dashed border-[#0F172A]/20" data-testid="dashboard-empty">कोई रिपोर्ट नहीं मिली</div>}
          {items.map((it) => {
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 text-sm text-slate-600 font-mono">
                      ...
                      <span>{d.toLocaleString('hi-IN')}</span>
                      <span className="mx-1">•</span>
                      <span className="font-bold text-[#0F172A]">{it.reporter_name}</span>
                    </div>
                    <div className="mt-1 text-lg text-[#0F172A]">{it.text || it.transcript || <em className="text-slate-500">कोई विवरण नहीं</em>}</div>
                    <div className="mt-2 flex gap-2 items-center">
                      {it.photo_url && <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 border border-slate-300">📷 फोटो</span>}
                      {it.audio_url && <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 border border-slate-300">🎤 आवाज़</span>}
                      {it.is_urgent && <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 border border-red-500 text-red-800 font-bold flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> तुरंत ज़रूरी</span>}
                    </div>
                  </div>
...
function DetailPanel({ inc, onClose, onUpdated }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success('अपडेट सेव हो गया');
      onUpdated(r.data);
    } catch {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center p-0 md:p-6" onClick={onClose}>
      <div className="bg-white w-full md:max-w-2xl md:rounded-3xl rounded-t-3xl border-2 border-[#0F172A] max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} data-testid="detail-panel">
        <div className="sticky top-0 bg-[#0F172A] text-white px-5 py-3 flex items-center justify-between rounded-t-3xl">
          <div className="font-heading text-lg">रिपोर्ट विवरण</div>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <Field label="रिपोर्टर" value={inc.reporter_name} />
            <Field label="समय" value={new Date(inc.created_at).toLocaleString('hi-IN')} />
            <Field label="स्थान" value={inc.location_label || '—'} />
            <Field label="जीपीएस" value={inc.gps_lat ? `${inc.gps_lat.toFixed(4)}, ${inc.gps_lng.toFixed(4)}` : '—'} />
          </div>
          {mapUrl && <a href={mapUrl} target="_blank" rel="noreferrer" className="text-[#D97706] underline text-sm" data-testid="map-link">मानचित्र पर देखें →</a>}
          {inc.is_urgent && <div className="p-2 bg-red-100 border-2 border-red-500 text-red-800 font-bold rounded-xl flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> तुरंत ज़रूरी</div>}
          ...
          {inc.audio_url && (
            <div>
              <div className="text-sm font-bold text-slate-600 mb-1">आवाज़ रिकॉर्डिंग</div>
              <audio src={inc.audio_url} controls className="w-full" />
            </div>
          )}
          {inc.transcript && (
            <div>
              <div className="text-sm font-bold text-slate-600 mb-1">आवाज़ का लिखित रूप (Whisper)</div>
          ...
          <div>
            <div className="text-sm font-bold text-slate-600 mb-1">स्थिति बदलें</div>
            <Select value={status} onValueChange={setStatus}>
          ...
          <div>
            <div className="text-sm font-bold text-slate-600 mb-1">कार्यालय नोट्स</div>
            <Textarea data-testid="office-notes" value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-24 border-2" placeholder="आंतरिक टिप्पणी…" />
          </div>
          <Button data-testid="save-update-btn" onClick={save} disabled={saving} className="w-full h-12 bg-[#D97706] text-white font-bold btn-tactile-amber rounded-2xl">{saving ? 'सेव हो रहा है…' : 'अपडेट सेव करें'}</Button>
        </div>
      </div>
    </div>
  );
